...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            "*** YOUR CODE HERE ***"
            # END Problem 13
            if isinstance(insect, QueenAnt) and insect.impostor:
                return
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while current_place.entrance != None and current_place != hive and current_place.bees == [] and counter < self.max_range:
            current_place = current_place.entrance
            counter += 1
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        "*** YOUR CODE HERE ***"
        current_place = self.place
        bee = [elem for elem in current_place.bees]
        result_armor = self.armor - amount
        if result_armor <= 0:
            for elem in bee:
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        # BEGIN Problem 7
        "*** YOUR CODE HERE ***"
        temp = [elem for elem in self.place.bees]
        for elem in temp:
            elem.reduce_armor(self.damage)
    ...
    def action(self, colony):
        ...
        if self.impostor == False:
            ...
            while current_place != None:
                if current_place.ant and not current_place.ant in self.hit_ant:
                    self.hit_ant += [current_place.ant]
                    current_place.ant.damage *= 2
                if current_place.ant and current_place.ant.container and current_place.ant.ant and not current_place.ant.ant in self.hit_ant:
                    self.hit_ant += [current_place.ant.ant]
...
def apply_effect(effect, bee, duration):
    ...
    "*** YOUR CODE HERE ***"
    temp = bee.action
    def action_apply(colony):
        nonlocal duration
        if duration == 0:
            return temp(colony)
        else:
            duration -= 1
            return effect(temp)(colony)
    bee.action = action_apply
    # END Problem EC
